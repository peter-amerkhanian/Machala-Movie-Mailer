...
from . import private_variables, email
import smtplib
...
for movie in movies:
    ...
    if english_bool:
        body = email.create_email(film, times, ratings)
        english_time = english_times[0]
        todays_movies_html += '<p><a href="{}"><b>{}</b></a> {}</p>'.format(film['trailer'], film['title'], english_time)
...
if len(todays_movies) >= 1:
    ...
    make_html_file(todays_movies, plural, todays_movies)
    # Create & send messages
    if __name__ == "__main__":
        msgs = []
        for to_address in private_variables.to_addresses:
            msgs.append(email.create_email_object(private_variables.from_address,
                                                  to_address,
